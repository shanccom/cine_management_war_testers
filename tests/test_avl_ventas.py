...
# --- AVL: Carnet de Extranjeria (longitud y categoria)
@pytest.mark.parametrize(
    "documento, expected_status",
    [
        ("92310123", "error"),         # 8 (min-1)
        ("923101234", "ok"),           # 9 (min CE)
        ("9231012345", "error"),       # 10 (max+1 sin sufijo)
        ("923101234R", "error"),       # sufijo 1 caracter
        ("923101234RG", "ok"),         # sufijo 2 caracteres
        ("923101234RGX", "error"),     # sufijo 3 caracteres
    ],
)
def test_avl_longitud_carnet_extranjeria(ventas_service: VentasService, documento: str, expected_status: str) -> None:
    payload = _base_payload(cliente_documento=documento)
    result = ventas_service.comprar_entrada(payload)
    assert result["status"] == expected_status
    if expected_status == "error":
...
def test_avl_cuarto_digito_tipo_carnet(ventas_service: VentasService, documento: str, expected_status: str) -> None:
    payload = _base_payload(cliente_documento=documento)
    result = ventas_service.comprar_entrada(payload)
    assert result["status"] == expected_status
    if expected_status == "error":
...
def test_avl_categoria_migratoria_carnet(ventas_service: VentasService, documento: str, expected_status: str) -> None:
    payload = _base_payload(cliente_documento=documento)
    result = ventas_service.comprar_entrada(payload)
    assert result["status"] == expected_status
    if expected_status == "error":
